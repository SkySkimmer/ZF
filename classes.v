...
Module Classes_axioms.

Parameter C : Type.

Axiom nonemptyC : nonemptyT C.

Parameter incC : C -> C -> Prop.

Definition subC x y := forall a, incC a x -> incC a y.

Axiom extensionalityC : forall c c', subC c c' -> subC c' c -> c=c'.

Definition is_set c := exists c', incC c c'.

Lemma set_is_set : forall c c', incC c c' -> is_set c.
Proof.
uhg. ir;exists c';am.
Qed.

Parameter ZC : (C -> Prop) -> C.

Axiom ZC_in : forall c, is_set c -> forall cp : C -> Prop, cp c -> incC c (ZC cp).

Axiom ZC_pr : forall c cp, incC c (ZC cp) -> cp c.

Definition emptyC := ZC (fun _ => False).

Lemma emptyC_empty : forall c, ~ incC c emptyC.
Proof.
ir. intro. 
apply (ZC_pr H).
Qed.

Lemma emptyC_sub_all : forall c, subC emptyC c.
Proof.
intros c H;ir.
apply ZC_pr in H0. nin H0.
Qed.

Lemma empty_emptyC : forall c, (forall x, incC x c -> False) -> c = emptyC.
Proof.
ir.
ap extensionalityC.
intro;ir. apply H in H0;nin H0.
ap emptyC_sub_all.
Qed.
...
Definition pair a b := doubleton (singleton a) (doubleton a b).

Lemma pair_eq_l : forall a b c d, pair a b = pair c d -> a=c.
Proof.
uf pair;ir.
assert (inc (singleton c) (doubleton (singleton a) (doubleton a b))).
rw H. ap doubleton_l.
apply doubleton_or in H0. nin H0.
ap singleton_inj. au.

unfold singleton in H0. rwi doubleton_eq H0.
nin H0;ee;subst. tv. au.
Qed.

Lemma pair_eq_r : forall a b c d, pair a b = pair c d -> b=d.
Proof.
ir.
unfold pair in H;unfold singleton in H.
repeat rwi doubleton_eq H.
repeat (match type of H with | or _ _ => nin H;ee;subst | _ => fail end);nin H0;ee;subst;tv.
Qed.

Lemma pair_eq : forall a b c d, pair a b = pair c d -> (a=c & b=d).
Proof.
ir. cp H.
apply pair_eq_l in H. apply pair_eq_r in H0. ee;am.
Qed.
...
Definition pr1 x := union (Z (union x) (fun a => exists b, x = pair a b)).
Definition pr2 x := union (Z (union x) (fun b => exists a, x = pair a b)).
...
Lemma pair_union : forall a b, union (pair a b) = doubleton a b.
Proof.
ir. ap extensionality;uhg;ir.
apply union_ex in H;nin H;ee.
apply doubleton_or in H;nin H;ee. subst. apply singleton_eq in H0;subst;ap doubleton_l.
subst. am.
apply doubleton_or in H. nin H;subst.
ap union_inc. exists (singleton a). ee. ap doubleton_l. ap singleton_inc.
ap union_inc. econstructor. ee. ap doubleton_r. ap doubleton_r.
Qed.
...
apply union_ex in H;nin H;ee. apply Z_all in H;ee.
nin H1. apply pair_eq in H1. ee;subst. am.

ap union_inc. econstructor. ee.
ap Z_inc. Focus 2. exists b. reflexivity.
ap doubleton_l. am.
Qed.
...
ap union_inc. econstructor. ee.
ap Z_inc. Focus 2. exists a. reflexivity.
ap doubleton_r. am.
Qed.
...
Lemma pr1_npr : forall x, ~ is_pair x -> P x = emptyset.
Proof.
ir.
ap empty_emptyset. ir.
ufi P H0. apply union_ex in H0;nin H0;ee. apply Z_all in H0;ee.
nin H2. ap H. rw H2. ap pair_is_pair.
Qed.

Lemma pr2_npr : forall x, ~ is_pair x -> Q x = emptyset.
Proof.
ir.
ap empty_emptyset. ir.
ufi Q H0. apply union_ex in H0;nin H0;ee. apply Z_all in H0;ee.
nin H2. ap H. rw H2. ap pair_is_pair.
Qed.

Definition product a b := Z (powerset (powerset (union2 a b))) (fun z => exists x, exists y, inc x a & inc y b & z = pair x y).
...
unfold pair in H1. apply doubleton_or in H1.
ap powerset_inc;uhg;ir.
 nin H1;subst.
apply singleton_eq in H2. subst. ap union2_l. am.
apply doubleton_or in H2. nin H2;subst;ap union2_inc;au.
exists x;exists y;ee;tv.
Qed.

Lemma product_pair_pr : forall a b x y, inc (pair x y) (product a b) -> 
(inc x a & inc y b).
Proof.
ir. apply Z_all in H. nin H.
nin H0;nin H0.
nin H0.  nin H1. apply pair_eq in H2. ee;subst;am.
Qed.
...
Proof.
ir. apply Z_pr in H.
nin H;nin H;nin H;nin H0. subst.
clpr. ee;au. ap pair_is_pair.
Qed.
...
Proof.
ir;ap iff_eq;ir.
union_nin H. Im_nin H.
subst. Im_nin H0. subst. exists x1;exists x0;ee;au.
